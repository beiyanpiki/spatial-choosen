...
import { loadAllChipConfigManifests, loadChipConfigData, type ChipConfigManifest } from '@/lib/preprocess/chipConfigs';
...
import { deriveSelectedSpotIdsFromRegions, buildSelectedCountSummary } from '@/lib/preprocess/tissueRegions';
import type {
  AlignmentSlice,
  CropQcSlice,
...
type PreprocessWorkspaceProps = {
  ...
  onProjectMutate: (updater: (current: PreprocessProject) => PreprocessProject) => void;
...
const placeholderCopyByStep: Record<PreprocessStepId, { title: string; body: string }> = {
  sourceAssets: {
...
const formatStatusText = (project: PreprocessProject) => {
  const eosin = project.sourceAssets.images.eosin?.fileName ?? 'missing';
  const he = project.sourceAssets.images.he?.fileName ?? 'missing';

  return [
    `Workflow version ${project.workflowVersion}`,
...
export function PreprocessWorkspace({
  ...
  const localizationImage = project
    ...
    : null;
  const alignmentReferenceImage = project ? project.sourceAssets.images[project.alignment.referenceImage] ?? null : null;
  const alignmentMovingImage = project ? project.sourceAssets.images[project.alignment.movingImage] ?? null : null;
  ...
  const applyLocalizationUpdate = useCallback((
    ...
    onProjectMutate((current) => {
      const nextLocalizationBase = normalizeLocalizationSlice(updater(current.localization));
      const nextHasImage = Boolean(current.sourceAssets.images[nextLocalizationBase.targetImage]?.dataUrl);
      const nextLocalization: LocalizationSlice = {
        ...nextLocalizationBase,
        method: nextLocalizationBase.method ?? (nextLocalizationBase.chipBounds ? 'manual' : null),
        status: computeLocalizationStatus(nextHasImage, nextLocalizationBase.chipBounds),
        isStale: false,
        error: null,
  ...
  const applyAlignmentUpdate = useCallback((
    ...
    onProjectMutate((current) => {
      const nextAlignmentBase = normalizeAlignmentSlice(updater(current.alignment));
  ...
  const handleUploadEosin = useCallback(async (fileList: FileList | null) => {
    const file = fileList?.[0];
    if (!file) return;

    try {
      const eosin = await buildSourceImage(file, 'eosin');
      const localizationAspectRatio = eosin.width && eosin.height ? eosin.width / eosin.height : 1;
      ...
      onProjectMutate((current) => {
        ...
        const nextLocalization: LocalizationSlice = {
          ...nextLocalizationBase,
          status: computeLocalizationStatus(true, nextLocalizationBase.chipBounds),
          isStale: false,
          error: null,
          updatedAt: timestamp,
        };
      ...
      toast({
        ...
        description: 'The image is ready for localization and downstream preprocess steps were invalidated.',
        status: 'success',
      });
    } catch (error) {
      console.error(error);
      toast({
        ...
        description: error instanceof Error ? error.message : 'Choose a browser-supported image file.',
        status: 'error',
      });
    }
  }, [onProjectMutate, toast]);

  const handleUploadHe = useCallback(async (fileList: FileList | null) => {
    const file = fileList?.[0];
    if (!file) return;
    ...
    try {
      ...
      toast({
        ...
        description: 'Landmark alignment can now target this image and downstream preprocess steps were invalidated.',
        status: 'success',
      });
    } catch (error) {
      console.error(error);
  ...
  useEffect(() => {
    ...
    onProjectMutate((current) => {
      const currentImage = current.sourceAssets.images[current.localization.targetImage];
      if (current.currentStep !== 'localization' || !currentImage?.dataUrl || current.localization.chipBounds) {
        return current;
      }

      const localizationAspectRatio = currentImage.width && currentImage.height ? currentImage.width / currentImage.height : 1;
  ...
  }, [currentStepId, hasLocalizationChipBounds, localizationImageDataUrl, onProjectMutate, project]);

  const currentStepMeta = project
    ? PREPROCESS_STEP_ITEMS.find((step) => step.id === project.currentStep) ?? PREPROCESS_STEP_ITEMS[0]
    : PREPROCESS_STEP_ITEMS[0];
  const currentCopy = project ? placeholderCopyByStep[project.currentStep] : placeholderCopyByStep.sourceAssets;

  const localizationSummaryJson = useMemo(() => {
    if (!project) return '{}';

    return JSON.stringify({
      targetImage: project.localization.targetImage,
      imageTransform: project.localization.imageTransform,
      chipBounds: project.localization.chipBounds,
      boxColor: project.localization.boxColor,
      downstream: {
        alignment: project.alignment.status,
        cropQc: project.cropQc.status,
        chipConfig: project.chipConfig.status,
        tissueSelection: project.tissueSelection.status,
        exportState: project.exportState.status,
      },
    }, null, 2);
  }, [project]);
  ...
  const runCropQcStep = useCallback(async () => {
    if (!project) return;
    if (!alignmentReferenceImage?.dataUrl || !alignmentMovingImage?.dataUrl || !project.localization.chipBounds || !project.alignment.affineMatrix) {
      toast({
        title: 'Crop prerequisites missing',
        description: 'Alignment and localization must be completed before crop/QC.',
        status: 'warning',
      });
      return;
    }
    ...
    try {
      ...
      toast({
        ...
        description: error instanceof Error ? error.message : 'Unable to generate crop previews.',
        status: 'error',
      });
    }
  }, [alignmentMovingImage, alignmentReferenceImage, applyCropQcUpdate, project, toast]);
  ...
  useEffect(() => {
    ...
    loadAllChipConfigManifests()
      ...
      .catch((error) => {
        if (!cancelled) {
          const message = error instanceof Error ? error.message : 'Failed to load chip manifests';
  ...
  if (isLoading) {
    return (
      <Flex minH='100vh' bg='gray.50' align='center' justify='center' direction='column' gap={4}>
  ...
  if (loadError || !project) {
    return (
      <Flex minH='100vh' bg='gray.50' align='center' justify='center' px={4}>
        <Box bg='white' border='1px solid' borderColor='red.100' boxShadow='md' borderRadius='lg' p={8} maxW='560px' w='100%'>
          <Stack spacing={4}>
            <Badge colorScheme='red' alignSelf='flex-start'>Unavailable</Badge>
            <Heading size='md'>Unable to open preprocess workspace</Heading>
            <Text color='gray.600'>{loadError ?? 'Project not found in this browser.'}</Text>
            <Button onClick={onBackToLanding} colorScheme='brand' alignSelf='flex-start'>
  ...
  return (
    <Flex direction='column' minH='100vh' bg='gray.50' data-testid='preprocess-workspace-shell'>
      <Flex flex='1' direction='column' px={{ base: 4, md: 10 }} py={{ base: 6, md: 8 }} gap={6}>
        <Flex justify='space-between' align={{ base: 'flex-start', lg: 'center' }} gap={4} wrap='wrap'>
          <Stack spacing={3}>
            <Button variant='ghost' px={0} alignSelf='flex-start' onClick={onBackToLanding}>
              ← Back to preprocess projects
            </Button>
            <Stack spacing={1}>
              <Heading size='lg'>Preprocess workspace</Heading>
              <Text color='gray.600' maxW='760px'>
                The preprocess shell now includes an interactive localization stage with autosaved image transforms and normalized chip bounds.
              </Text>
            </Stack>
          </Stack>

            <Stack spacing={2} align={{ base: 'stretch', lg: 'flex-end' }}>
              <Text fontSize='sm' color='gray.500'>Autosave</Text>
            <Badge
              ...
              {autosaveDetail ? <Text fontSize='xs' color='gray.500'>{autosaveDetail}</Text> : null}
            </Stack>
          </Flex>

        <Flex direction={{ base: 'column', lg: 'row' }} gap={6} align='stretch'>
          <StepSidebar currentStep={project.currentStep} onStepSelect={onStepChange} project={project} />

          <Box flex='1' bg='white' border='1px solid' borderColor='gray.100' borderRadius='lg' boxShadow='md' p={{ base: 5, md: 6 }}>
            <Stack spacing={6}>
              <Flex justify='space-between' align={{ base: 'stretch', md: 'center' }} gap={4} wrap='wrap'>
                <Stack spacing={1}>
                  ...
                  <Text color='gray.600'>{currentCopy.body}</Text>
                </Stack>
              </Flex>
              ...
              {project.currentStep === 'localization' ? (
                <Flex direction={{ base: 'column', xl: 'row' }} gap={6} align='stretch'>
                  <LocalizationPanel
                    ...
                    summaryJson={localizationSummaryJson}
                  />
                ...
                <CropQcPanel
                  ...
                  checkerboardDataUrl={project.cropQc.checkerboardPreviewDataUrl}
                  ...
                  canAccept={Boolean(project.cropQc.cropWidth && project.cropQc.cropHeight)}
                />
                      ...
                      try {
                        ...
                        onProjectMutate((current) => ({
                          ...
                          tissueSelection: {
                            ...
                            overrideNotice: current.tissueSelection.forcedInSpotIds.length > 0 || current.tissueSelection.forcedOutSpotIds.length > 0 || current.tissueSelection.regions.length > 0
                        ...
                        const message = error instanceof Error ? error.message : 'Failed to load chip config';
                        setChipConfigError(message);
                        onProjectMutate((current) => ({
                          ...current,
                          chipConfig: {
                ...
                <Stack spacing={4}>
                  <HStack spacing={3} align='center'>
                    <Text fontSize='sm' color='gray.600'>Activation threshold</Text>
                    <Input
                      width='120px'
                      type='number'
                      min={0}
                      ...
                      onChange={(event) => {
                        ...
                        onProjectMutate((current) => ({
                          ...
                          tissueSelection: {
                            ...current.tissueSelection,
                            activationThreshold: Number.isFinite(next) ? next : current.tissueSelection.activationThreshold,
                            status: 'ready',
                            warning: null,
                            isStale: false,
                            updatedAt: new Date().toISOString(),
                    ...
                    <Button
                      ...
                      colorScheme='brand'
                      onClick={() => {
                        if (!project.cropQc.eosinPreviewDataUrl || !project.chipConfig.projectedSpots) return;
                        const eosinCropDataUrl = project.cropQc.eosinPreviewDataUrl;
                        ...
                        void (async () => {
                          ...
                          onProjectMutate((current) => {
                            const hasRegions = current.tissueSelection.regions.length > 0;
                            const finalSelectedSpotIds = hasRegions
                              ? deriveSelectedSpotIdsFromRegions(current.tissueSelection.regions, projectedSpots)
                              : applySpotOverrides({
                                  autoSelected: result.selectedIds,
                                  forcedIn: current.tissueSelection.forcedInSpotIds,
                                  forcedOut: current.tissueSelection.forcedOutSpotIds,
                                });
                            const paritySummary = buildSelectedCountSummary(finalSelectedSpotIds, projectedSpots.length);
                            ...
                            return {
                              ...
                              tissueSelection: {
                                ...
                                status: hasRegions ? 'complete' : result.warning ? 'error' : 'complete',
                                isStale: false,
                                updatedAt: new Date().toISOString(),
                              ...
                              exportState: {
                                ...current.exportState,
                                status: hasRegions ? 'stale' : result.warning ? 'stale' : 'ready',
                  ...
                  <HStack justify='space-between'>
                    ...
                    <Text data-testid='tissue-selected-count' fontWeight='semibold'>
                      {project.tissueSelection.paritySummary?.selectedCount ?? 0}
                    </Text>
                  </HStack>
                  <HStack justify='space-between'>
                    <Text color='gray.600'>Selected percent</Text>
                    <Text data-testid='tissue-selected-percent' fontWeight='semibold'>
                      {project.tissueSelection.paritySummary?.selectedPercent.toFixed(2) ?? '0.00'}
                    </Text>
                  </HStack>
                  <Box as='pre' fontSize='xs' color='gray.700' bg='gray.50' borderRadius='md' p={2} data-testid='tissue-parity-json'>
                    {JSON.stringify({
                      thresholdMode: project.tissueSelection.thresholdMode,
                      activationThreshold: project.tissueSelection.activationThreshold,
                      blockThreshold: project.tissueSelection.blockThreshold,
                  ...
                  {project.tissueSelection.warning ? <Text color='orange.700'>{project.tissueSelection.warning}</Text> : null}
                  {project.tissueSelection.overrideNotice ? <Text color='orange.700'>{project.tissueSelection.overrideNotice}</Text> : null}
                  ...
                  <TissueSelectionPanel
                    ...
                    selectedSpotIds={(() => {
                      ...
                      const projectedSpots = project.chipConfig.projectedSpots ?? [];
                      if (regions.length > 0) {
                        return deriveSelectedSpotIdsFromRegions(regions, projectedSpots);
                      }
                    ...
                    onRegionsChange={(nextRegions) => {
                      ...
                      onProjectMutate((current) => {
                        ...
                        const paritySummary = buildSelectedCountSummary(finalSelectedSpotIds, totalSpots);
                        return {
                          ...current,
                          tissueSelection: {
                            ...current.tissueSelection,
                  ...
                  onDownload={() => {
                    ...
                    void (async () => {
                      ...
                      try {
                        ...
                        toast({
                          title: 'Export failed',
                          description: error instanceof Error ? error.message : 'ZIP export failed',
                          status: 'error',
                        });
                        onProjectMutate((current) => ({
                          ...current,
              ...
              <Box border='1px solid' borderColor='gray.100' borderRadius='lg' p={4} bg='gray.50'>
                <Stack spacing={2}>
                  <Heading size='sm'>Current saved context</Heading>
                  {formatStatusText(project).map((line) => (
                    <Text key={line} fontSize='sm' color='gray.600'>{line}</Text>
                  ))}
