...
type AlignmentPanelProps = {
  ...
  onUploadMovingImage?: (fileList: FileList | null) => void;
};
...
type LandmarkCanvasProps = {
  ...
  onCreatePoint: (imageKey: 'source' | 'target', point: PreprocessPoint) => void;
  onDeletePoint: (id: string) => void;
  onMovePoint: (id: string, imageKey: 'source' | 'target', point: PreprocessPoint) => void;
  testIdPrefix: string;
};
...
const makePointId = () => (typeof crypto !== 'undefined' && crypto.randomUUID
  ? crypto.randomUUID()
  : `align-${Date.now()}-${Math.random().toString(16).slice(2)}`);

const formatPercent = (value: number | null) => (value === null ? '—' : `${(value * 100).toFixed(1)}%`);
const formatMetric = (value: number | null, digits = 3) => (value === null ? '—' : value.toFixed(digits));
const ZOOM_MIN = 0.75;
const ZOOM_MAX = 50;
...
function LandmarkCanvas({
  ...
  const baseView = useMemo(
    () => computeBaseView(viewport, Math.max(1, (image.width ?? 1) / Math.max(1, image.height ?? 1))),
  ...
  const toImagePoint = useCallback((clientX: number, clientY: number) => {
    ...
    const normalized = relativeToImage({ x: clientX - rect.left, y: clientY - rect.top }, viewportTransform);
  ...
  const beginPan = useCallback((clientX: number, clientY: number, pointerId: number) => {
  ...
  const updatePan = useCallback((clientX: number, clientY: number, pointerId: number) => {
  ...
  const endPan = useCallback((clientX: number, clientY: number, pointerId: number) => {
    const session = panSessionRef.current;
    if (!session || session.pointerId !== pointerId) return;
  ...
  useEffect(() => {
    ...
    const handleWheel = (event: WheelEvent) => {
      ...
      const next = computeZoomTransform(baseView, zoom + (event.deltaY > 0 ? -0.2 : 0.2), anchorNorm ?? undefined, anchorNorm ? anchorScreen : undefined);
      if (!next) return;
  ...
  const renderPoint = useCallback((point: PreprocessPoint) => {
    ...
    return {
      ...
      y: viewportTransform.originY + displayPoint.y * viewportTransform.height,
    };
  }, [imageTransform, viewportTransform]);
  ...
  return (
    <Stack spacing={3} flex='1' minW={0}>
      <HStack justify='space-between' align='center'>
        <Text fontSize='sm' fontWeight='semibold' color='gray.600'>{title}</Text>
        <HStack spacing={2} minW='190px'>
          <Text fontSize='xs' color='gray.500' whiteSpace='nowrap'>Zoom</Text>
          <Input
            type='range'
            min={ZOOM_MIN}
            max={ZOOM_MAX}
      ...
      >
        <Box
          ...
          position='absolute'
          inset={0}
          data-testid={testIdPrefix === 'alignment-source' ? 'alignment-add-point-eosin' : testIdPrefix === 'alignment-target' ? 'alignment-add-point-he' : `${testIdPrefix}-canvas-host`}
        >
          {viewportTransform ? (
            <>
              <Box
                  ...
                  return (
                    <g key={point.id}>
                      <circle
                        ...
                        cy={rendered.y}
                        r={8}
                        fill={point.isInlier ? 'rgba(56,161,105,0.92)' : 'rgba(229,62,62,0.92)'}
                        stroke='white'
                        strokeWidth={1.5}
                      ...
                      />
                      <text x={rendered.x + 11} y={rendered.y - 11} fontSize='12' fill='white'>
                        {index + 1}
                      </text>
                    </g>
                  );
...
export function AlignmentPanel({
  ...
  const [pendingPair, setPendingPair] = useState<PendingPair>({ source: null, target: null });
  ...
  const hasBothImages = Boolean(referenceImage?.dataUrl && movingImage?.dataUrl);
    ...
    const load = async () => {
      try {
        ...
        if (!cancelled) {
          ...
          setRuntimeError(error instanceof Error ? error.message : 'Failed to initialize OpenCV runtime');
        }
      }
    };
    load();
  ...
  const resetSolveState = useCallback((current: AlignmentSlice): AlignmentSlice => ({
    ...current,
    inlierMask: null,
    affineMatrix: null,
  ...
  const addOrUpdatePendingPoint = useCallback((imageKey: 'source' | 'target', point: PreprocessPoint) => {
    ...
    onAlignmentChange((slice) => {
      const hasDuplicatePair = slice.controlPoints.some((existingPoint) => (
  ...
  const handleMovePoint = useCallback((id: string, imageKey: 'source' | 'target', point: PreprocessPoint) => {
    onAlignmentChange((slice) => {
      const nextSlice: AlignmentSlice = {
        ...slice,
  ...
  const sourcePoints = useMemo<EditorPoint[]>(() => alignment.controlPoints.map((pair, index) => ({
    id: pair.id,
    point: pair.source,
    isInlier: alignment.inlierMask?.[index] ?? true,
  })), [alignment.controlPoints, alignment.inlierMask]);

  const targetPoints = useMemo<EditorPoint[]>(() => alignment.controlPoints.map((pair, index) => ({
    id: pair.id,
    point: pair.target,
    isInlier: alignment.inlierMask?.[index] ?? true,
  })), [alignment.controlPoints, alignment.inlierMask]);
  ...
  const solveAlignment = useCallback(async (forceMode = false) => {
    if (!referenceImage?.width || !referenceImage.height || !movingImage?.width || !movingImage.height) return;
    ...
    try {
      ...
      onAlignmentChange((current) => ({
        ...
        status: 'error',
        error: error instanceof Error ? error.message : 'Alignment solve failed',
        isStale: false,
        updatedAt: new Date().toISOString(),
      }));
    }
  }, [alignment.controlPoints, chipBounds, movingImage?.dataUrl, movingImage?.height, movingImage?.width, onAlignmentChange, referenceImage?.dataUrl, referenceImage?.height, referenceImage?.width]);

  const movingUploadRef = useRef<HTMLInputElement | null>(null);
      ...
      <Box border='1px solid' borderColor='orange.200' borderRadius='lg' p={5} bg='orange.50'>
        <Stack spacing={2}>
          ...
          <Text color='orange.800' fontSize='sm'>
            Eosin and H&E images must be present before landmark pairing and alignment solving can run.
          </Text>
          {!movingImage?.dataUrl && onUploadMovingImage ? (
            <>
              <Button size='sm' alignSelf='flex-start' onClick={() => movingUploadRef.current?.click()}>
                Upload H&E image
              </Button>
              <Input
                ref={movingUploadRef}
                ...
                display='none'
                onChange={(event) => {
                  onUploadMovingImage(event.target.files);
                  event.target.value = '';
                }}
              />
            </>
    ...
    <Stack spacing={5}>
      <Stack spacing={2}>
        ...
        <HStack spacing={3} wrap='wrap'>
          <Badge
            colorScheme={alignment.controlPoints.length >= ALIGNMENT_MIN_PAIRS ? 'green' : 'orange'}
          ...
          <Badge
            colorScheme={runtimeStatus === 'ready' ? 'green' : runtimeStatus === 'error' ? 'red' : 'orange'}
          ...
          <Badge
            colorScheme={alignment.solveAccepted ? 'green' : alignment.failureReason ? 'red' : 'gray'}
          ...
          >
            {alignment.solveAccepted ? 'Accepted' : alignment.failureReason ? 'Rejected' : 'Not solved'}
          </Badge>
        </HStack>
        {coverage.warning ? (
          <Text fontSize='sm' color='orange.600' data-testid='alignment-distribution-warning'>
            Landmark spread is narrow. Coverage ratios are {formatPercent(coverage.coverageRatioX)} width and {formatPercent(coverage.coverageRatioY)} height,
            below the {Math.round(ALIGNMENT_COVERAGE_THRESHOLD * 100)}% minimum.
          </Text>
        ) : null}
        {runtimeError ? (
          <Text fontSize='sm' color='red.600'>{runtimeError}</Text>
        ) : null}
      </Stack>
      ...
      <HStack spacing={3} align='center' wrap='wrap'>
        ...
        <Button
          ...
          onClick={() => {
            setPendingPair({ source: null, target: null });
            onAlignmentChange((slice) => resetSolveState({
              ...slice,
              controlPoints: [],
              inlierMask: null,
            }));
      ...
      {alignment.failureReason && !alignment.solveAccepted && (
        <Box border='1px solid' borderColor='red.200' borderRadius='lg' p={4} bg='red.50'>
          <Stack spacing={2}>
            ...
            <Text fontSize='sm' color='red.700'>
              ...
              Using &ldquo;Force continue&rdquo; will skip RANSAC outlier detection and compute
              the transformation using all control points. This may result in precision
      ...
      <Stack spacing={3} maxW='320px'>
        <Text fontSize='sm' fontWeight='semibold' color='gray.600'>H&amp;E transform</Text>
        <Text fontSize='sm' color='gray.500'>Display-only transform for the moving image.</Text>
        <Stack spacing={1}>
          <Text fontSize='xs' color='gray.500'>Rotation</Text>
          <Input
            type='range'
            min={-180}
            max={180}
            ...
            onChange={(event) => {
              ...
              onAlignmentChange((slice) => normalizeAlignmentSlice({
                ...slice,
            ...
            px={0}
          />
          <Text fontSize='xs' color='gray.500'>{alignment.movingImageTransform.rotationDegrees.toFixed(1)}°</Text>
        </Stack>
        <Stack spacing={1}>
          <Text fontSize='xs' color='gray.500'>Scale</Text>
          <Input
            type='range'
            min={0.5}
            max={4}
            ...
            px={0}
          />
          <Text fontSize='xs' color='gray.500'>{(alignment.movingImageTransform.scale * 100).toFixed(0)}%</Text>
        </Stack>
      ...
      <Stack spacing={2} fontSize='sm'>
        <HStack justify='space-between'>
          ...
          <Text fontWeight='semibold' data-testid='alignment-inlier-ratio'>{formatPercent(alignment.inlierRatio)}</Text>
        </HStack>
        <HStack justify='space-between'>
          ...
          <Text fontWeight='semibold' data-testid='alignment-rmse'>
            ...
            {alignment.ransacReprojThreshold !== null ? ` (threshold ${formatMetric(ALIGNMENT_RMSE_MULTIPLIER * alignment.ransacReprojThreshold)} px)` : ''}
          </Text>
        </HStack>
        <HStack justify='space-between'>
          <Text color='gray.600'>Quality gates</Text>
          <Text fontWeight='semibold'>
            minPairs:{alignment.qualityFlags.minPairs ? '✓' : '✗'} • inlier:{alignment.qualityFlags.inlierRatio ? '✓' : '✗'} • rmse:{alignment.qualityFlags.rmse ? '✓' : '✗'} • matrix:{alignment.qualityFlags.finiteMatrix ? '✓' : '✗'} • scale:{alignment.qualityFlags.scaleRange ? '✓' : '✗'}
          </Text>
        </HStack>
        <Box as='pre' fontSize='xs' color='gray.700' bg='gray.50' borderRadius='md' p={2} data-testid='alignment-matrix-json'>
          {alignment.affineMatrix ? JSON.stringify(alignment.affineMatrix) : 'null'}
        </Box>
      </Stack>
    </Stack>
  );
