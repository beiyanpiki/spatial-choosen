...
const normalizeQuad = (value: unknown): HeFocusAutoProposal['refinedQuad'] => {
  ...
  return normalizedPoints.every((point) => point !== null)
    ? [normalizedPoints[0], normalizedPoints[1], normalizedPoints[2], normalizedPoints[3]]
    : null;
};
...
const normalizeHeFocusAutoProposal = (
  ...
  return {
    status: autoProposal.status === 'accepted'
      || autoProposal.status === 'fallback'
      || autoProposal.status === 'failed'
      ? autoProposal.status
      : defaults.status,
    method: typeof autoProposal.method === 'string' && autoProposal.method.length > 0
      ? autoProposal.method
      : null,
    ...
    eccCorrelation: isFiniteNumber(autoProposal.eccCorrelation)
      ...
      : null,
    failureReason: typeof autoProposal.failureReason === 'string'
...
export const createHeFocusSlice = (status: PreprocessSliceBase['status']): HeFocusSlice => ({
  ...createSlice(status),
  targetImage: 'he',
  chipBounds: null,
  handles: [],
...
export const normalizeHeFocusSlice = (slice: HeFocusSlice | LegacyHeFocusSlice): HeFocusSlice => {
...
export const buildEmptyPreprocessProject = (name: string): PreprocessProject => {
