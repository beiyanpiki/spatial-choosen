...
const clearAttemptState = (globalObject: OpenCvGlobal) => {
  document
    .querySelectorAll<HTMLScriptElement>('script[data-opencv-runtime-admin="true"]')
...
const createRuntimeWaiter = (
  ...
  const checkReady = () => {
    const runtime = isReady(globalObject.cvBuiltInAdmin)
      ? globalObject.cvBuiltInAdmin
      : isReady(moduleObject)
        ? moduleObject
        : null;
    if (!runtime) return;
    globalObject.cvBuiltInAdmin = runtime;
    finish();
  };
...
const appendScriptTag = (): Promise<void> => new Promise((resolve, reject) => {
  ...
  script.async = true;
  script.dataset.opencvRuntimeAdmin = 'true';
...
const runLoadAttempt = async (globalObject: OpenCvGlobal) => {
  ...
  if (!isReady(globalObject.cvBuiltInAdmin)) {
...
export async function loadOpenCv(): Promise<{ cv: OpenCvRuntime }> {
  ...
  if (isReady(globalObject.cvBuiltInAdmin)) {
    return { cv: globalObject.cvBuiltInAdmin };
  }

  if (!globalObject.__opencvScriptPromiseBuiltInAdmin) {
    globalObject.__opencvScriptPromiseBuiltInAdmin = initializeOpenCv(globalObject).catch((error) => {
      const loadError = toError(error, 'Failed to initialize OpenCV runtime');
      globalObject.__opencvLoadErrorBuiltInAdmin = loadError;
  ...
  await globalObject.__opencvScriptPromiseBuiltInAdmin;
