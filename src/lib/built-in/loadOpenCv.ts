...
type OpenCvGlobal = {
  cvBuiltin?: OpenCvModule;
  ModuleBuiltin?: OpenCvModule;
  __opencvScriptPromiseBuiltin?: Promise<void>;
  __opencvLoadErrorBuiltin?: Error;
};
...
const clearAttemptState = (globalObject: OpenCvGlobal) => {
  ...
  globalObject.cvBuiltin = undefined;
  globalObject.ModuleBuiltin = undefined;
};
...
const createRuntimeWaiter = (
  ...
  const checkReady = () => {
    const runtime = isReady(globalObject.cvBuiltin)
      ? globalObject.cvBuiltin
      : isReady(moduleObject)
        ? moduleObject
        : null;
    if (!runtime) return;
    globalObject.cvBuiltin = runtime;
    finish();
  };

  globalObject.ModuleBuiltin = moduleObject;
...
const runLoadAttempt = async (globalObject: OpenCvGlobal) => {
  ...
  if (!isReady(globalObject.cvBuiltin)) {
    throw new Error('OpenCV runtime was not ready after initialization');
  }

  if (globalObject.ModuleBuiltin) {
    globalObject.ModuleBuiltin.wasmBinary = undefined;
  }
};
...
const initializeOpenCv = async (globalObject: OpenCvGlobal) => {
  ...
  for (let attempt = 1; attempt <= OPENCV_MAX_ATTEMPTS; attempt += 1) {
    ...
    try {
      ...
      globalObject.__opencvLoadErrorBuiltin = undefined;
      return;
    } catch (error) {
...
export async function loadOpenCv(): Promise<{ cv: OpenCvRuntime }> {
  ...
  if (isReady(globalObject.cvBuiltin)) {
    return { cv: globalObject.cvBuiltin };
  }

  if (!globalObject.__opencvScriptPromiseBuiltin) {
    globalObject.__opencvScriptPromiseBuiltin = initializeOpenCv(globalObject).catch((error) => {
      const loadError = toError(error, 'Failed to initialize OpenCV runtime');
      globalObject.__opencvLoadErrorBuiltin = loadError;
      globalObject.__opencvScriptPromiseBuiltin = undefined;
      clearAttemptState(globalObject);
      throw loadError;
    });
  }

  await globalObject.__opencvScriptPromiseBuiltin;
