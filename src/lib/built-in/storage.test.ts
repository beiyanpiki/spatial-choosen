...
import { normalizeProjectForPersistence, normalizeProjectForWorkspace } from '@/app/built-in/projectState';
...
describe('preprocess storage tissue metadata', () => {
  ...
  it('hydrates legacy PNG working previews without regenerating them', async () => {
    ...
    const stored = JSON.parse(localStorage.getItem('spatial-builtin-projects') ?? '[]') as Array<{
      sourceAssets: {
        images: {
    ...
    localStorage.setItem('spatial-builtin-projects', JSON.stringify(stored));
  ...
  it('persists canonical matrix and support metadata while stripping runtime selectedSpotIds', async () => {
    ...
    const stored = JSON.parse(localStorage.getItem('spatial-builtin-projects') ?? '[]') as Array<Record<string, unknown>>;
  ...
  it('restores prior metadata when a tissue payload write fails', async () => {
    ...
    vi.stubGlobal('window', {
      localStorage,
      indexedDB: createIndexedDbMock(
        (storeName) => failTissueWrite && storeName === 'builtin-tissue-selection',
      ),
    });
    const project = createProject();
    project.storageVersion = PREPROCESS_STORAGE_SCHEMA_VERSION;
    await upsertPreprocessProject(project);
    const previousMetadata = localStorage.getItem('spatial-builtin-projects');
    ...
    await expect(upsertPreprocessProject(project, { mode: 'tissue' })).rejects.toThrow(
      'Forced builtin-tissue-selection write failure',
    );

    expect(localStorage.getItem('spatial-builtin-projects')).toBe(previousMetadata);
  });
  ...
  it('hydrates pre-v5 stored metadata without projectedSpotIndex by repairing a compact index from chip config data', async () => {
    ...
    localStorage.setItem('spatial-builtin-projects', JSON.stringify([legacyStored]));
