...
type TestOpenCvWindow = typeof window & {
  cvBuiltin?: TestOpenCvModule;
  ModuleBuiltin?: TestOpenCvModule;
...
const completeCurrentAttempt = () => {
  ...
  const moduleObject = globalObject.ModuleBuiltin;
  ...
  globalObject.cvBuiltin = moduleObject;
...
describe('loadOpenCv', () => {
  ...
  it('prefetches WASM and resolves from the runtime callback', async () => {
    ...
    expect(getOpenCvWindow().ModuleBuiltin?.wasmBinary).toBeInstanceOf(ArrayBuffer);

    completeCurrentAttempt();

    const result = await loadPromise;
    expect(result.cv).toBe(getOpenCvWindow().cvBuiltin);
    ...
    expect(getOpenCvWindow().ModuleBuiltin?.wasmBinary).toBeUndefined();
  });
  ...
  it('clears an aborted runtime and automatically retries with a fresh script', async () => {
    ...
    await expect(loadPromise).resolves.toEqual({ cv: getOpenCvWindow().cvBuiltin });
  ...
  it('allows a later call to retry after both automatic attempts abort', async () => {
    ...
    await expect(retryLoad).resolves.toEqual({ cv: getOpenCvWindow().cvBuiltin });
  });
});
