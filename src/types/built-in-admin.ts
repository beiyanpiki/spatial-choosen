...
export type ChipConfigSlice = PreprocessSliceBase & {
  ...
  /** Spot diameter in chip-config units (50um: 50, 15um: 25), populated from the tissue activation CSV import. */
  spotDiameter: number | null;
  /** 1-based in-memory array rows excluded at capture-area definition. arrayRow 1 = image top (template convention). */
  excludedRows: number[];
  ...
  /** File name of the tissue activation CSV that fixed the chip configuration. */
